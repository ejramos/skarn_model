%file: skarn_generic.m

%author:  Evan J. Ramos

%Description: Generic skarn model
close all
clear
clc
...
T_crit   = Tsol;              %[K] temperature at which pluton becomes permeable %%%%REVIEWER suggestion
...
if pluton ~= 0
    [Mineral, phi, Kd, Pluton, k] = make_medium(filename,Grid,sett,pluton);
else
    [Mineral, phi, Kd,~,k] = make_medium(filename,Grid,sett,pluton);       
end
phi_m                      = vphi_m*(1-phi);
...
while t < endtime
    ...
    if exist('Pluton','var')
        ...
        Param.O_w.dof_dir   = qdown_dof;
        Param.O_w.dof_f_dir = qdown_dof_f;
        Param.O_w.g         = Rwi*ones(length(qdown_dof),1);
        Param.O_w.dof_neu   = [];
        Param.O_w.dof_f_neu = [];
        Param.O_w.qb        = [];
    end
